...
# ---------- LLM ----------
def deepseek_chat(messages: List[Dict[str,str]], model: str = DEEPSEEK_MODEL,
                  max_tokens: int = 600, temperature: float = 0.2, timeout: int = 60) -> str:
    ...
    r = rq.post(url, headers=hdr, json=payload, timeout=timeout)
    r.raise_for_status()
    j = r.json()
